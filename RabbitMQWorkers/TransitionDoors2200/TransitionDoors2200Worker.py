...
def main():
    ...
    channel.queue_declare(queue='TransitionDoors2200', durable=True)

    # Bind the queue to the exchange to receive MQTT messages.
    channel.queue_bind(exchange='amq.topic', queue='TransitionDoors2200', routing_key='TransitionDoors2200')
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            blob_client = container_client.get_blob_client(blob='TransitionDoors2200/incoming' + str(datetime.datetime.now()) + '.txt')
    ...
    channel.basic_consume(
        queue='TransitionDoors2200', on_message_callback=callback, auto_ack=False)
    
    # Start consuming messages from the queue.
    print(' [*] TransitionDoors2200 worker waiting for messages. To exit press CTRL+C')
