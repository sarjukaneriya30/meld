...
def main():
    ...
    # Connect to 'ConveyorD365' queue.
    channel = connection.channel()
    channel.queue_declare(queue='ConveyorD365', durable=True)

    # Bind the queue to the exchange to receive MQTT messages.
    channel.queue_bind(exchange='amq.topic', queue='ConveyorD365', routing_key='ConveyorD365')
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            blob_client = container_client.get_blob_client(blob='Conveyor/incoming' + str(datetime.datetime.now()) + '.txt')
        ...
        print('Received message, stored in blob storage: {}'.format(body), flush=True)

    # Start consuming messages from the queue.
    channel.basic_consume(
        queue='ConveyorD365', on_message_callback=callback, auto_ack=False)
    
    # Start consuming messages from the queue.
    print(' [*] ConveyorD365 worker waiting for messages. To exit press CTRL+C')
