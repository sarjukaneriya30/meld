...
def main():
    ...
    channel.queue_declare(queue='D365OneRFID', durable=True)

    # Bind the queue to the exchange to receive MQTT messages.
    channel.queue_bind(exchange='amq.topic', queue='D365OneRFID', routing_key='D365OneRFID')
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            blob_client = container_client.get_blob_client(blob='D365OneRFID/incoming' + str(datetime.datetime.now()) + '.txt')
    ...
    channel.basic_consume(
        queue='D365OneRFID', on_message_callback=callback, auto_ack=False)
    
    # Start consuming messages from the queue.
    print(' [*] D365OneRFID worker waiting for messages. To exit press CTRL+C')
