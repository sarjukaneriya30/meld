...
def main():
    ...
    channel.queue_declare(queue='TransitionDoors1330', durable=True)

    # Bind the queue to the exchange to receive MQTT messages.
    channel.queue_bind(exchange='amq.topic', queue='TransitionDoors1330', routing_key='TransitionDoors1330')
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            blob_client = container_client.get_blob_client(blob='TransitionDoors1330/incoming' + str(datetime.datetime.now()) + '.txt')
    ...
    channel.basic_consume(
        queue='TransitionDoors1330', on_message_callback=callback, auto_ack=False)
    
    # Start consuming messages from the queue.
    print(' [*] TransitionDoors1330 worker waiting for messages. To exit press CTRL+C')
