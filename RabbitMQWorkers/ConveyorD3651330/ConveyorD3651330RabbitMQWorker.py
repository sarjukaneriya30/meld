...
def main():
    ...
    channel.queue_declare(queue='ConveyorD3651330', durable=True)

    # Bind the queue to the exchange to receive MQTT messages.
    channel.queue_bind(exchange='amq.topic', queue='ConveyorD3651330', routing_key='ConveyorD3651330')
    ...
    def callback(ch, method, properties, body):
        try:
            ...
            blob_client = container_client.get_blob_client(blob='Conveyor1330/incoming' + str(datetime.datetime.now()) + '.txt')
    ...
    channel.basic_consume(
        queue='ConveyorD3651330', on_message_callback=callback, auto_ack=False)
    
    # Start consuming messages from the queue.
    print(' [*] ConveyorD3651330 worker waiting for messages. To exit press CTRL+C')
