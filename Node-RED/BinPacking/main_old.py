...
def main(items=None):
    return_statment = '' # build a string to return
    ...
    return_statment = return_statment + f'Number of items: {n_items}\n'
    #print(f'Number of bins used: {n_bins}')
    return_statment = return_statment + f'Number of bins used: {n_bins}\n'
    #print(f'Bins used: {", ".join(all_bin_names)}')
    return_statment = return_statment + f'Bins used: {", ".join(all_bin_names)}\n'
    #print('Result:')
    return_statment = return_statment + 'Result:\n'
    #print('='*25)
    return_statment = return_statment + ('='*25) + '\n'
    for idx, this_bin in enumerate(all_bins_content):
        #print(this_bin['ContainerType']+':')
        return_statment = return_statment + this_bin['ContainerType']+':\n'
        #print(f'Number of items: {this_bin["n_items"]}')
        return_statment = return_statment + f'Number of items: {this_bin["n_items"]}\n'
        #print('Items:')
        return_statment = return_statment + 'Items to pack:\n'
        # i=0
        for item in this_bin['items']:
            #print(item)
            return_statment = return_statment + '- ' + item + '\n'
            # if i%2!=0:
            #     #print()
            # i+=1
    ...
    #         'n_items': n_items,
    #         'n_bins': n_bins,
    #         'bins_used': all_bin_names,
    #         'result': all_bins_content
    #         }
...
if __name__ == '__main__':
    with open('/data/temp.json') as f:
        data = json.load(f)
    print(main(items=data), file=sys.stdout) # this is what we want to go to stdout
    # #print(json.dumps(main(items=data), indent=4), file=sys.stdout)
