...
def get_d365_inventory_df(dbcon=None):
    ...
    query = '''
        ...
        p.CONTAINERTYPEID AS "ParentContainerType",
        s.WAREHOUSELOCATIONID
        FROM "InventInventoryOnhandReportStorageV2Staging" s
        LEFT JOIN "WHSLicensePlateStaging" l on s.LICENSEPLATENUMBER = l.LICENSEPLATENUMBER
...
def get_tx_history_df(search_back_minutes=35):
...
def get_nearest_staging_bin_to_location(locid):
    '''
    ...
    StagingBinLocs = {'1A':'(79, 10)','2A':'(79, 34)'} # new from JG 11/3; updated 2/8
...
#     print(location_tup)
    ...
    return  'I21'+aisle+col+'AA'



def get_location_by_lpn(lpn, dbcon=None):
    '''
    If location for this LPN was previously assigned,
    return that location.  Else, return None.
    '''

    dd = get_tx_history_df()
    print('Calling get_d365_inventory_df within get_location_by_lpn')
    df = get_d365_inventory_df(dbcon=dbcon)

    if lpn in df.LPN:
        return df[df.LPN==lpn].WAREHOUSELOCATIONID.iloc[0]

    elif lpn in dd.LoadIdentifier:
        return dd[dd.LoadIdentifier == lpn]
...
def storage_opt(signal, dbcon=None):
    ...
    try:
        if action == 'Stage':
            ...
            stage_bin = get_nearest_staging_bin_to_location(final_location)
            ...
            # we will search for this LPN in the full history, for all of time
            dd = get_tx_history_df(search_back_minutes=100*365*24*60) # tx history for 100 years

            lpn = signal['LoadIdentifier']
            # if the LPN is found in the forever history, it has been staged before
            if lpn in dd.LoadIdentifier:
                ddd = dd[dd.LoadIdentifier == lpn]

                # this will be raised if the LPN exists in the history more than once.  This is a conflict.
                if len(ddd) > 1:
                    raise ValueError(f'Fatal - there are too many entries for this LPN in the tx history. Expected 1, got {len(dd)}.')

                StaginLoc = ddd.iloc[0].StagingLoc
                FinalLoc = ddd.iloc[0].FinalLoc

                return FinalLoc

            else:
                return assign_loc_and_return_modified_signal(signal)[1]
