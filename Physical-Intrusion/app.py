...
video_path = './captures/'
time_interval = 60 # how often the page reloads in seconds
    ...
    dbc.Row([
        dbc.Col([
            ...
            dash_table.DataTable(
                ...
                html.Div([
                dbc.Button(
                'Commit Review Notes to Database',
                id='commit-notes',
                size='sm',
                className='kpmg-button'
                )
                ], style={'textAlign':'right', 'padding-top':'0.5rem'}),
...
@app.callback(Output('cv-results-table', 'data'),
              ...
              Output('cv-results-table', 'sort_by'),
              Input('url', 'pathname'),
              Input('refresh-interval', 'n_intervals'))
def load_cv_table(path, refresh):
    if path:
        ...
        excluded_cols = ['filename', 'Review Status', 'id']
        ...
        # reset sort
        sort_by = [
            {'column_id': 'Detection Time', 'direction': 'desc'}
            ]
        ...
        return table_data, columns, dropdown, tooltip_header, style_header_conditional, sort_by
    raise PreventUpdate
...
def update_selection(data, refresh, row):
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if 'refresh-interval' in changed_id: # reset on page refresh
        print('Resetting row selection on refresh...')
        return [0]
    if not row:
        print('Starting default row selection...')
        return [0]
    raise PreventUpdate
...
def most_recent_detection(data):
    if data:
        ...
        review = df[df['Review Status']=='REQUIRES REVIEW'].sort_values('Detection Time', ascending=False)
        if not review.empty:
...
@app.callback(Output('write-to-database', 'children'),
              Input('commit-notes', 'n_clicks'),
              State('cv-results-table', 'data'))
def update_database(btn, data):
    changed_id = [p['prop_id'] for p in dash.callback_context.triggered][0]
    if 'commit-notes' in changed_id and data:
        # for attempt in range(10): # try a few times if there's an error
        #     print('Attempt #{} to update row with notes'.format(attempt))
        #     try:
        ...
        comparison = df[['id', 'Review Status']].merge(df2[['id', 'Review Status']], left_on='id', right_on='id', how='outer', suffixes=['_Origin', '_Dash'])
        comparison = comparison[comparison['Review Status_Origin']!=comparison['Review Status_Dash']]

        for idx, row in comparison.iterrows():
            row_id = row['id']
            new_status = row['Review Status_Dash']
